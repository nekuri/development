...
composer self-update && composer create-project --prefer-dist "cakephp/app=3.*" ${ROOT} << END_OF_INPUT
y
END_OF_INPUT
...
replace="\n\$__env = env('CAKE_ENV', 'production');\nConfigure::load(\$__env, 'default');"
replace="${replace}\nif (\$__env === 'production') {\n    Configure::write('debug', false);\n}"
sed -i -e "s@//Configure::load('app_local', 'default');@//Configure::load('app_local', 'default');${replace}@" ${BOOTSTRAP}
